...
Response::Response(int code, std::string status, std::vector<std::shared_ptr<MessageHeader>> headers)
    : statusCode_(code), statusDesc_(std::move(status)), SIPMessage(std::move(headers)) {
  headers_.emplace_back(std::make_shared<MessageHeader>("Content-Length", "0"));
}

Response::Response(int code, std::string status, std::vector<std::shared_ptr<MessageHeader>> headers,
                   const std::string &body)
    : statusCode_(code), statusDesc_(std::move(status)), SIPMessage(std::move(headers)), body_(body) {
  headers_.emplace_back(std::make_shared<MessageHeader>("Content-Length", std::to_string(body.size())));
  headers_.emplace_back(std::make_shared<MessageHeader>("Content-Type", "application/sdp"));
}
...
std::shared_ptr<SIPMessage> Response::genReply(int code) {
  return std::make_shared<Response>(100, "Trying", std::vector<std::shared_ptr<MessageHeader>>());
}
